...
export default function DangerousClausesReport({ clauses }) {
  ...
  // ONLY show HIGH risk clauses - these are truly dangerous
  const dangerousClauses = clauses.filter(c => c.risk === "High");
  ...
  return (
    <div className="glass-card">
      ...
      {/* Dangerous Clauses - ONLY HIGH RISK */}
      <div className="space-y-3 mb-6">
        {dangerousClauses.map((clause) => (
          <div 
            key={clause.id}
            className="border-2 border-red-500/50 rounded-lg overflow-hidden hover:border-red-500 transition-all shadow-lg"
          >
            {/* Header - Always Visible */}
            <div 
              className="p-4 bg-gradient-to-r from-red-900/30 to-pink-900/30 cursor-pointer flex items-center justify-between"
            ...
            >
              <div className="flex items-center gap-4 flex-1">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-300 line-clamp-1">{clause.explanation}</p>
                </div>
              </div>
              
              <button className="ml-4 p-2 hover:bg-red-800/30 rounded-lg transition-colors">
      ...
      {/* No safe clauses section - only show stats */}
      {/* Quick Stats */}
